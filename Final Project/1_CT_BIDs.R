...
#Clean data
#Note that I'm excluding Census Tracts that have less than 1% of its area within a BID.
clean <- raw %>% 
  select(-date_modif, -OBJECTID_1, -FID_geo_ex, -FID_geo__1, -ntaname, -ntacode, -ctlabel, -shape_leng, -cdeligibil, -puma, - Shape_Le_1, -Shape_Area  ) %>%  
  filter(!is.na(ct2010), !is.na(bid), a_weight > 0.01) %>%
  ...
  mutate (BID_id = as.character(objectid),
          ...
          countyfips = case_when(
            boro_name == "Bronx"  ~ "05",  
            boro_name == "Brooklyn"  ~ "47",
            boro_name == "Manhattan"  ~ "61",
            boro_name == "Queens"  ~ "81",
            boro_name == "Staten Island"  ~ "85",
            TRUE ~ " "
          ),
          CT_id_full = glue('360{countyfips}{ct2010}'),
          BID_name = bid,
          BID_date = date_creat,
  ...
  ungroup() %>% 
  select (CT_id_full, areaCT_ft, CT_a_weight, BID_dummy,
          BID_id, BID_name, areaBID_ft, boro_name)
...
#75 BIDs in raw file, 71 BIDs in clean file. 
...
#330 unique CT, this is equal to number of obs in the dataset. 

#Export clean file
exp_path<- glue("{data_dir}/Clean/CT_BIDs.csv") 
clean %>% write.csv(exp_path, na="")
...
clean_CT<-raw %>%
  ...
  select (CT_id_full, areaCT_ft, boro_name) %>% 
  ungroup()

#Full join both datasets
full_CT<- clean_CT %>% full_join(clean, by=c("CT_id_full", "areaCT_ft", "boro_name"))

#Export full CT file
exp_path2<- glue("{data_dir}/Clean/CT_BIDs_Full.csv") 
full_CT %>% write.csv(exp_path2, na="")
